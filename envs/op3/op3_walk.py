...
from tasks import walking_task_op3
from envs.common import mujoco_env
from envs.common import robot_interface
from envs.op3 import robot

from .gen_xml import builder


class Op3WalkEnv(mujoco_env.MujocoEnv):
    def __init__(self):
        sim_dt = 0.0025
        control_dt = 0.025
        frame_skip = (control_dt/sim_dt)

        path_to_xml_out = '/tmp/mjcf-export/op3_walk/op3.xml'
        ...
        coeff = 1
        pdgains.T[0] = coeff * np.array([50, 50, 50, 50, 50, 50,
                                         50, 50, 50, 50, 50, 50, ])
        ...
        # set up task
        self.task = walking_task_op3.WalkingTask(client=self.interface,
                                             dt=control_dt,
