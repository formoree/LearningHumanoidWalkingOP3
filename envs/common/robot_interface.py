...
class RobotInterface(object):
    def __init__(self, model, data, rfoot_body_name=None, lfoot_body_name=None, root_name = 'PELVIS_S'):
        self.model = model
        self.data = data
        ...
        self.robot_root_name = root_name

        self.stepCounter = 0

    def nq(self):
        return self.model.nq # 返回坐标

    def nu(self):
        return self.model.nu # 返回actuators

    def nv(self):
        return self.model.nv # 返回速度
    ...
    def get_actuated_joint_inds_op3(self):
        """
        Returns list of joint indices to which actuators are attached.
        """
        joint_names = [mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_JOINT, i) for i in range(self.model.njnt)]
        actuator_names = [mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_ACTUATOR, i) for i in range(self.model.nu)]
        return [idx for idx, jnt in enumerate(joint_names) if jnt+'_act' in actuator_names]
    ...
    def get_actuated_joint_names_op3(self):
        """
        Returns list of joint names to which actuators are attached.
        """
        joint_names = [mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_JOINT, i) for i in range(self.model.njnt)]
