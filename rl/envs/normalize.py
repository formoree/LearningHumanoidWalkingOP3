...
def _run_random_actions(iter, policy, env_fn, noise_std):
    """
    Ray框架装饰的远程函数，用于在模拟环境中执行随机动作并收集观察数据。
    @ray.remote ：将函数标记为 Ray 远程函数，允许它在分布式环境中执行。
    Args:
        iter: 迭代次数
        policy: 用于生成动作的策略
        env_fn: 创建环境的函数
        noise_std: 添加到动作的高斯噪声的标准差
    """
    env = WrapEnv(env_fn) # 创建并包装环境实例
...
def get_normalization_params(iter, policy, env_fn, noise_std, procs=4):
    """
    function: 并行计算得到policy的平均值和标准差

    Args:
        iter: 总迭代次数
        policy: 当前策略
        env_fn: 创建环境的函数
        noise_std: 添加到动作的噪声标准差
        procs: 并行进程数，默认为4
    ...
    Others：
        np.sqrt(np.var(states, axis=0) + 1e-8)：
            1. 加入一个小的常数项以确保数值稳定性，防止在接下来的平方根运算中出现除以零或非常小的数的情况
            2. np.var(states, axis=0) 指定沿着第0轴（通常是行）计算方差
