...
class PPO:
    ...
    def update_policy(self, obs_batch, action_batch, return_batch, advantage_batch, mask, mirror_observation=None, mirror_action=None):
        ...
        action_batch: 动作批次
        return_batch: 回报批次
        advantage_batch: 优势批次
        mask: 掩码（可能用于处理不同长度的序列）
        mirror_observation: 镜像观察（可选，用于对称学习）
        mirror_action: 镜像动作（可选，用于对称学习）
        """
        policy = self.policy
        critic = self.critic
    ...
    def train(self,
        ...
        self.policy = policy
        self.critic = critic
        # ps 是 "epsilon" 的缩写，通常表示一个很小的正数， 防止除零
        ...
        for itr in range(n_itr):
            ...
            for epoch in range(self.epochs):
                ...
                if self.recurrent:
                    """
                    创建一个 SubsetRandomSampler，范围是从 0 到 len(batch.traj_idx)-1
                    【使用 SubsetRandomSampler 的目的是为了在每个 epoch 随机打乱数据顺序，这有助于防止过拟合并提高模型的泛化能力。】
                    使用这个采样器创建一个 BatchSampler，不丢弃最后一个不完整的批次
                    """
                    ...
                    """
                    创建一个 SubsetRandomSampler，范围是从 0 到 num_samples-1
                    使用这个采样器创建一个 BatchSampler，丢弃最后一个不完整的批次
                    """
