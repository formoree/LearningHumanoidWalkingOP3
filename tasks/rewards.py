...
def _calc_torque_reward(self, prev_torque):
    ...
    """
    这个奖励函数的设计考虑了以下几个方面：
        1. 扭矩平滑性：鼓励扭矩的平滑变化，惩罚突然的大幅度变化。
        2. 能量效率：通过减少扭矩的剧烈变化，间接鼓励更高的能量效率。
        3. 归一化：通过取平均值，使奖励与关节数量无关。
        4. 非线性映射：使用指数函数提供非线性的奖励映射。
    这种奖励设计的目的是：
        1. 鼓励机器人产生更平滑、更自然的运动。
        2. 减少执行器的剧烈动作，可能有助于延长硬件寿命。
        3. 间接促进能量效率，因为大的扭矩变化通常与高能耗相关。
    通过调整参数（如乘数 0.25 或使用不同的奖励函数），可以进一步优化奖励函数以适应特定需求。例如：
        1. 增大乘数会使系统对扭矩变化更敏感。
        2. 减小乘数会允许更大的扭矩变化。
    """
...
def _calc_height_reward(self):
    # 高度奖励
    # 确定接触点高度：检查脚是否与地面接触；如果有接触，取所有接触点中最低的 z 坐标作为接触点高度；如果没有接触，接触点高度设为 0。
...
def _calc_foot_frc_clock_reward(self, left_frc_fn, right_frc_fn):
    ...
    # 设置最大期望的足部力量：设置为机器人质量的一半乘以重力加速度。
    ...
    # 获得时钟，使用正切函数计算得分，这会放大接近理想值的情况，惩罚偏离的情况
...
def _calc_body_orient_reward(self, body_name, quat_ref=[1, 0, 0, 0]):
    ...
    body_quat = self._client.get_object_xquat_by_name(body_name, "OBJ_BODY") #当前朝向
    target_quat = np.array(quat_ref) # 目标朝向
    error = 10 * (1 - np.inner(target_quat, body_quat) ** 2) # 内积计算误差 *10来放大误差
    return np.exp(-error)
